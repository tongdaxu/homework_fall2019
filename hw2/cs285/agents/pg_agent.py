...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = [np.power(self.gamma, t) for t in indices]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [rewards[idx] * discounts[idx] for idx in indices]
        ...
        list_of_discounted_returns = [sum_of_discounted_rewards for _ in range(len(rewards) - 1)]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            indices = range(start_time_index, len(rewards))

            # 2) create a list where the entry at each index (t') is gamma^(t'-t)
            discounts = [np.power(self.gamma, idx - start_time_index) for idx in indices]
            ...
            discounted_rtg = [rewards[idx] * discounts[idx] for idx in range(len(indices))]
